...
class MCMC:
    def __init__(self, get_model, dim: int) -> None:
        ...
        self.median_parameters = None
        self.acceptance_ratio = 0

    # obtain the chi squared value of the model y-values given current parameters vs. the measured y-values
    def get_log_likelihood(self, params: np.ndarray, x: np.ndarray, y: np.ndarray, error: np.ndarray) -> float:
    ...
    def step_eval(self, params: np.ndarray, step_size: np.ndarray, x: np.ndarray, \
                y: np.ndarray, error: np.ndarray, perturbed_value_index: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        chi_squared_old = self.get_log_likelihood(params, x, y, error)
        ...
        chi_squared_new = self.get_log_likelihood(try_param, x, y, error)
    ...
    def MCMC(self, params: np.ndarray, step_size: np.ndarray, x: np.ndarray, \
            y: np.ndarray, error: np.ndarray, n_steps: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        for i in range(0, n_steps):
            ...
            new_params, accepted, chi_squared = self.step_eval(params, step_size, x, y, error, param_index)
        ...
        self.bayesian_information_criterion = self.dim * np.log(len(x)) + chi_squared_chain[-1]

        # calculate 68% CIs for all parameters on the side
        stacked_chain = np.column_stack((chain, chi_squared_chain))
        stacked_chain = stacked_chain[stacked_chain[:, 2].argsort()]
        ...
        # calculate acceptance ratio on the side
        self.acceptance_ratio /= n_steps

        return chain, accept_chain, chi_squared_chain
    
    def calculate_confidence_intervals(self, index_to_cut):
        # all entries to the left of index_to_cut are chopped off
        self.confidence_intervals = []
        self.chain = self.chain[index_to_cut:,]
        self.accept_chain = self.accept_chain[index_to_cut:,]
        self.chi_squared_chain = self.chi_squared_chain[index_to_cut:,]
        stacked_chain = np.column_stack((self.chain, self.chi_squared_chain))
        stacked_chain = stacked_chain[stacked_chain[:, 2].argsort()]
        stacked_chain_truncated = stacked_chain[: int(len(stacked_chain) * 0.68)]

        # also calculate median model parameters
        self.median_parameters = stacked_chain_truncated[len(stacked_chain_truncated) // 2]
    ...
    def get_samples(self):
        # ensure the chain is trimmed first - call calculate_confidence intervals
        indices = np.random.choice(range(len(self.chain)), 100)
        samples = np.zeros((100, self.dim))

        for i in range(0, len(indices)): # should be 100 - hardcoded
            samples[i] = self.chain[i]
